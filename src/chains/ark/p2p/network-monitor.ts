...
import { ApplicationEvents } from "../event-emitter/";
...
export class NetworkMonitor implements P2P.INetworkMonitor {
    ...
    public async start(): Promise<void> {
        ...
        if (this.config.skipDiscovery) {
            ...
            for (const [version, peers] of Object.entries(groupBy(this.storage.getPeers(), "version"))) {
                const castedPeers = peers as P2P.IPeer[];
                this.logger.info(`Discovered ${pluralize("peer", castedPeers.length, true)} with v${version}.`);
            }
        }
    ...
    public async syncWithNetwork(
        ...
        try {
            ...
            )) as Interfaces.IBlockData[][]).reduce((acc, curr) => [...acc, ...(curr || [])], []);
    ...
    public async broadcastBlock(block: Interfaces.IBlock): Promise<void> {
        console.log(`BROADCASTING BLOCK: ${block.data.id}`);
...
*/
    ...
    private async populateSeedPeers(): Promise<any> {
        ...
        const nanoConfig = app.getConfig().get("nano");
        const useAnchor: boolean = nanoConfig.useAnchor;
        const anchorPeer: IPeerData = nanoConfig.anchor;
        if(useAnchor)
        {
            anchorPeer.version = app.getVersion();
        ...
        return Promise.all(
            Object.values(peers).map((peer: P2P.IPeer) => {
                ...
                const isAnchor = useAnchor && peer.ip === anchorPeer.ip;
                if(isAnchor)
                    this.storage.setAnchor(peer);
