...
import { EventEmitter, Logger, State, P2P } from "../interfaces";

import { isBlockChained, roundCalculator } from "../core-utils";
...
const networkMonitor = app.resolvePlugin<P2P.IPeerService>("p2p").getMonitor();
...
blockchainMachine.actionMap = (blockchain: Blockchain) => ({
    ...
    downloadFinished() {
        ...
        console.log(stateStorage.getLastBlockIds().length)
        if (stateStorage.networkStart) {
    ...
    async init() {
        try {
            const lastHeight = networkMonitor.getNetworkHeight() - 1;
            let blockData: Interfaces.IBlockData[] = (await networkMonitor.syncWithNetwork(lastHeight, 50)).sort((a,b) => b.height - a.height);
            let blocks: Interfaces.IBlock[] = blockData.map(b => Blocks.BlockFactory.fromData(b));
            const block: Interfaces.IBlock = blocks[blocks.length - 1];
            /*
            ...
            */
            ...
            if (stateStorage.networkStart) {
                ...
                await blockchain.p2p.getMonitor().start(); <--network started before blockchain
                */
...
*/
            ...
            logger.info(`Last block retrieved from network: ${block.data.height.toLocaleString()}`);
